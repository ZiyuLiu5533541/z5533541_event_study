...
date = [value for key,value in aud_usd_lst]
aud_usd_series = pd.Series(date=date, index=index)
print(aud_usd_series)

index = [key for key,value in eur_aud_lst]
date = [value for key,value in ear_aud_lst]
eur_aud_series = pd.Series(date=date, index=index)
