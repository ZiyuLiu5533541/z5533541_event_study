...
def read_dat(tic):
    ...
    #go through DATDIR and find file that matches "tic" parameter
    # e.g. if tic = "aal", you need to find "aal_prc.dat"
    # open tic_prc.dat file and add every line into a list
...
# ----------------------------------------------------------------------------
    #"""For ensuring the code could be transplanted accurately as well as be executed from the correct location without
    #modifying.
    #Setting the location of files and folders also maintain the consistency of different modules within the same module.

    #Hypothesis 1 is more likely to be true. Hypothesis 1: Negative sentiment may indicate the negative estimation from
    #investors. The subsequent actions from investors will be related to the stock fluctuations.
    #Hypothesis 2: Negative words will have an impact on the stock in the short term, however, in the long run, investors
    #would re-examine the fundamentals of companies, so that there would be no long-term volatility and impact.
